...
rappi_mcp_project/
│
├── Docs/                           # Documentación y otros
│   └── init.sql                    # Script de creación de tablas
│   └── (otros archivos: diagramas, explicaciones, etc.)
│
├── mcp-server/
│   ├── Dockerfile                   # Para contenerizar el servidor MCP Python
│   ├── pyproject.toml               # Configuración del proyecto (dependencias, script)
│   └── main.py                      # Código del servidor MCP (tools)
│
├── docker-compose.yml               # Solo para el servicio Python (tu base de datos ya corre aparte)
├── .env                             # Variables de entorno (conexión a Postgres)
